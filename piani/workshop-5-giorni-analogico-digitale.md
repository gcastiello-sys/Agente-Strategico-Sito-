...
```mermaid
flowchart LR
  subgraph DIG["DIGITALE — fa alzare la mano (SCALA)"]
    A["YouTube<br/>biblioteca / asset H24"]
    B["Organico IG-FB<br/>autorità, non scala"]
    C["Ads<br/>volume prevedibile"]
  end
  subgraph ANA["ANALOGICO — converte la mano alzata"]
    D["Chiamata<br/>speed-to-lead (minuti)"]
    E["Referral / Centri d'influenza"]
    F["Eventi / Open House / Liste"]
  end
  A --> G["LEAD venditore"]
  B --> G
  C --> G
...
```mermaid
flowchart TD
  G1["GIORNO 1<br/>Il cambio di paradigma<br/>analogico → digitale"]
  G2["GIORNO 2<br/>Il sistema scalabile<br/>organico · YouTube · ads"]
  G3["GIORNO 3<br/>Contenuti che cercano<br/>i VENDITORI (non i like)"]
  G4["GIORNO 4<br/>Modulo AI agentico +<br/>l'analogico che converte"]
  G5["GIORNO 5<br/>La macchina completa +<br/>il prossimo livello (€4.000)"]
  G1 --> G2 --> G3 --> G4 --> G5
```
...
```mermaid
flowchart LR
  M["Video manifesto<br/>+ organico"] --> S["Sito"]
  S --> B["€149<br/>Bootcamp"]
  B --> W["Workshop 5 giorni<br/>(questo)"]
  W --> O["€4.000<br/>12 Masterclass + workshop avvio"]
  O --> C["Community / Sistema Dominante"]
```
